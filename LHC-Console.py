...
def generate_event(id1, id2, beam_energy, particles_list, probabilities, valid_particles, resonances, max_attempts=100000):
    ...
    C = api.get_particle_by_mcid(2212)
    Z = api.get_particle_by_mcid(2112)
    ...
    for attempt in range(max_attempts):
        ...
        try:
            for particle in api.get_particle_by_name(chosen_resonances.name).exclusive_branching_fractions():  
                ...
                #print([p.name for p in decay_products])
...
def main():
    print('╔════════════════════════════════════════════════╗')
    print('║      LHC-SIMULATOR (for hydrons) By Kleyman    ║')
    print('╚════════════════════════════════════════════════╝')
    time.sleep(0.05)
    print('Loading data base...')
    particle_list, resonances = load_particles()
    print('==================================================')
    print("  This simulator use PDG Id's")
    print('  example: proton(2212) neutron(2112) pion(211)')
    print('==================================================')
    id_1 = int(input('First hydron (ID): '))
    id_2 = int(input('Second hydron (ID): '))
    E = int(input('Energy(GeV, for example: 60): '))

    A = api.get_particle_by_mcid(id_1)
    B = api.get_particle_by_mcid(id_2)
    m1 = safe_mass(A)
    m2 = safe_mass(B)
    s = m1**2 + m2**2 + 2 * m2 * E
    sqrt_s = sqrt(max(0.1, s))
    prob, plist = [], [] #get_weigths(particle_list, sqrt_s)
    print(particle_list)
    print('\n' + '='*50)
    print(f'  Generating events for sqrt_s ~ {sqrt(2*E):.1f} GeV')
    print('='*50)

    total_time = 0

    # Генерируем несколько событий для демонстрации
    print("\nGenerated events:")
    print("-" * 50)

    for i in range(10):  # Покажем 10 событий
        event_start = time.time()
        event = generate_event(id_1, id_2, E, particle_list, prob, plist, resonances)
        ...
        if event:
            ...
            print(f"Event {i+1}: Failed to generate")
        
        total_time += event_time if event else event_time
